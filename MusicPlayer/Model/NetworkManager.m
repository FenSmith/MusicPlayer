    ...
    [manager GET:playlistURL parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if ([type isEqualToString:@"r"]) {
            ...
            if ([appDelegate.playList count] != 0) {
                appDelegate.currentSongIndex = [appDelegate.playList count] - 1;
...
//                [appDelegate.player play];
        ...
        [self.delegate reloadTableviewData];
        [[[Global GlobalSingleton] get_obj_home] refreshUI];
        if( [type isEqualToString:@"s"] ){
            NSNotification *notification = [NSNotification notificationWithName:@"playNextSong" object:self userInfo:nil];
